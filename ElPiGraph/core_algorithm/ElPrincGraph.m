...
%    of nodes and not cubically)
    ...
    if verbose
        display(sprintf(['BARCODE\tTIME\tENERGY\tNNODES\tNEDGES\tNRIBS\tNSTARS'...
    ...
    while (graph.nNodes < NumNodes)
        ...
        if report
            ...
            BARCODES(i) = {bcode};
            if verbose
                display(sprintf('%s\t%f\t%f\t%i\t%i\t%i\t%i\t%i\t%i\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f',...
                    char(BARCODES(i)), TIME(i), ENERGY(i), NNODES(i), NEDGES(i),...
    ...
    if nargout > 2
        ...
        URN2 = URN2(1:i); 
        URSD = URSD(1:i);
        ReportTable = struct('BARCODE', BARCODE, 'TIME', TIME, 'ENERGY', ENERGY,...
